...
void OcrLite::setNumThread(int numOfThread) {
    m_ctx->setNumThread(numOfThread);
}
...
void OcrLite::setGpuIndex(int gpuIndex) {
    m_ctx->setGpuIndex(gpuIndex);
}

bool OcrLite::initModels(const std::string &detPath, const std::string &clsPath,
                         const std::string &recPath, const std::string &keysPath) {
    Logger("=====Init Models=====\n");
    m_ctx->initModels(detPath, clsPath, recPath, keysPath);
...
OcrResult OcrLite::detect(const char *path, const char *imgName,
    ...
    std::vector<TextBox> textBoxes = m_ctx->dbNet.getTextBoxes(src, scale, boxScoreThresh, boxThresh, unClipRatio);
    ...
    angles = m_ctx->angleNet.getAngles(partImages, path, imgName,
                                       doAngle, mostAngle);
    ...
    std::vector<TextLine> textLines = m_ctx->crnnNet.getTextLines(partImages, path, imgName);
    //Log TextLines
