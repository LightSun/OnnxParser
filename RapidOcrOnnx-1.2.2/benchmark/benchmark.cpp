...
void test_ocr(){
    ...
    int loopCount = 1;
    int padding = 10;
    int maxSideLen = 1024; //raw 1024
    float boxScoreThresh = 0.5f;
    float boxThresh = 0.3f;
    float unClipRatio = 0.5f;
    bool doAngle = true; //启用或者禁用文字方向检测
    int flagDoAngle = 1;
    bool mostAngle = true; //角度投票(整张图片以最大可能文字方向来识别)
    int flagMostAngle = 1;
    int flagGpu = -1;
    //
    String md_dir = "/media/heaven7/Elements_SE/study/work/MedQA/meinian/RapidOcr/";
//    String imgDir = "/media/heaven7/Elements_SE/study/work/OCR/local/";
//    String imgName = "test.png";
    String imgDir = "/media/heaven7/Elements_SE/study/work/MedQA/"
                    "meinian/imgs/器官图像/";
    String imgName = "shen.png";
    //
    String modelDetPath = md_dir + "ch_PP-OCRv4_det_infer.onnx";
    String modelClsPath = md_dir + "ch_ppocr_mobile_v2.0_cls_infer.onnx";
    String modelRecPath = md_dir + "ch_PP-OCRv4_rec_infer.onnx";
    String keysPath = md_dir + "ppocr_keys_v1.txt";
    //
//    String modelDetPath = md_dir + "ec_PP-OCRv3_det_infer.onnx";
//    String modelClsPath;
//    String modelRecPath = md_dir + "en_PP-OCRv4_rec_infer.onnx";
//    String keysPath = md_dir + "en_dict.txt";
//    doAngle = false;
    //
    OcrLite ocrLite;
    ...
    bool initModelsRet = ocrLite.initModels(modelDetPath, modelClsPath, modelRecPath, keysPath);
    if (!initModelsRet){
        printf("ocrLite >> init failed.\n");
        return;
    }
    printf("=====Warmup 2 cycles=====\n");
    for (int i = 0; i < 2; ++i) {
        OcrResult result = ocrLite.detect(imgDir.c_str(), imgName.c_str(),
                                          padding, maxSideLen,
                                          boxScoreThresh, boxThresh,
                                          unClipRatio,
                                          doAngle, mostAngle);
